...
const Nav = (activeNav, setActiveNav) => {
  ...
  const DownArrow = () => {
    return (
      <svg width='24' height='24' xmlns='http://www.w3.org/2000/svg'>
        <defs>
          <path id='a' d='M18.59 16.41L20 15l-8-8-8 8 1.41 1.41L12 9.83' />
        </defs>
        <g fill='none' fill-rule='evenodd'>
          <mask id='b' fill='#fff'></mask>
          <g mask='url(#b)' fill='#636060'>
  ...
  const stickyNavHandler = () => {
    window.scrollY >= 54.4 ? setStickyNav(true) : setStickyNav(false);
  };
  ...
  return (
    <nav className={`navbar ${stickyNav ? 'sticky' : ''} `}>
      ...
      {!showSearch ? (
        <ul className='nav-left'>
          <li className=' nav-item'>
            <button className='nav-button' onClick={handleNavPhoneClick}>
              Phones
              <i className='material-icons nav-arrow'>expand_more</i>
            </button>
          </li>
          <li className=' nav-item'>
        ...
        <SearchBar closeSearch={handleShowSearch} />
      )}
      <ul className='nav-right'>
        ...
        {showAccountModal ? (
          <AccountModal closeModal={handleShowAccountModal} />
        ) : null}
      </ul>
    </nav>
  );
